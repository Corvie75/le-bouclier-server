...
io.on('connection', socket => {
  ...
  socket.on('action', ({ code, type, targetId, extra }) => {
    ...
    // Mage intercept — si la carte est As ou Roi sur TOUTE action, demander au mage
    const mage = room.players.find(p => p.heroId==='mage' && !p.eliminated && p.id!==actor.id);
...
function handleAction(room, actor, type, targetId, drawn, extra) {
  ...
  if (aType==='attack') {
    ...
    if (!isOgreKill && atkVal<=shieldVal) {
      ...
      // Paladin riposte — 2 attaques séquentielles
      if (target.heroId==='paladin'&&!target.eliminated&&totalPV(target)>0&&didDamage) {
        ...
        for (let r=0;r<2;r++) {
          ...
          if (rc.numVal>rsv) {
            const rpv=totalPV(actor);
            applyDamage(room,actor,rc.numVal-rsv);
            if (totalPV(actor)<rpv&&actor.charges.length>0){actor.charges.forEach(c=>room.discard.push(c));actor.charges=[];}
            paladinMsg += `Attaque ${r+1}: ${cStr(rc)} (${rc.numVal}) → −${rc.numVal-rsv} PV. `;
          } else {
            paladinMsg += `Attaque ${r+1}: ${cStr(rc)} (${rc.numVal}) bloqué. `;
          }
          room.discard.push(rc);
        }
        broadcastPopup(room,'🛡️⚔️',`${target.name} riposte !`, paladinMsg.trim(),null,'neutral');
        checkEliminated(room,actor);
        if (checkGameOver(room)) { room.discard.push(drawn); actor.drawnCard=null; return; }
      }
    }
