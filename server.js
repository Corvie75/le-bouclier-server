  ...
  return {
    players: room.players.map(p => {
      ...
      return {
        ...
        pv:p.pv, // cartes PV visibles par tous
    ...
    espionneInGame,
    waitingFor: room.waitingFor || null, // qui bloque le jeu
  };
}
...
function handleAction(room, actor, type, targetId, drawn, extra) {
  ...
  if (aType==='attack') {
    ...
    if (!isOgreKill && atkVal<=shieldVal) {
      ...
      // Potion feu 2e cible — avec popup dédié
      if (actor.potionFeu && actor.potionFeuTarget2) {
        ...
        if (t2 && t2.id!==targetId) {
          ...
          if (atkVal>sv2) {
            ...
            if (pv2After<pv2Before && t2.charges.length>0) { t2.charges.forEach(c=>room.discard.push(c)); t2.charges=[]; }
            broadcastPopup(room,'🔥',`Potion de Feu — ${t2.name} aussi !`,`${cStr(drawn)} (${atkVal}) contre bouclier ${getShieldVal(t2)+dmg2}`,`−${dmg2} PV → ${pv2After} PV`,'dmg');
            checkEliminated(room,t2);
          } else {
            broadcastPopup(room,'🔥🛡️',`Potion de Feu — ${t2.name} bloque !`,`${cStr(drawn)} (${atkVal}) contre bouclier ${sv2}`,'Bloqué !','block');
          }
        }
      ...
      // Paladin riposte — timing identique à une action normale
      // Action normale : reveal(2s) → anim(0.8s) → popup(3.2s) = 5.5s
      // On attend 5.5s que l'action principale soit terminée, puis on enchaîne
      if (target.heroId==='paladin'&&!target.eliminated&&totalPV(target)>0&&didDamage) {
        ...
        setTimeout(()=>{
          // Riposte 1 : reveal
          io.to(room.code).emit('card_reveal',{ card:rc1, chargeCard:null, actorName:target.name, actionLabel:`🛡️⚔️ Riposte 1 sur ${actor.name}`, isCharge:false });
          // Popup après 2s (comme action normale)
          setTimeout(()=>{
            ...
            // Riposte 2 après 5.5s (même timing)
            setTimeout(()=>{
              io.to(room.code).emit('card_reveal',{ card:rc2, chargeCard:null, actorName:target.name, actionLabel:`🛡️⚔️ Riposte 2 sur ${actor.name}`, isCharge:false });
              setTimeout(()=>{
        ...
        },5500); // attendre fin action principale
