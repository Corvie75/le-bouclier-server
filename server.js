...
function findCardOfValue(room, val) {
  // Chercher d'abord en défausse
  const idx = room.discard.findIndex(c => c.numVal === val);
  if (idx !== -1) return room.discard.splice(idx, 1)[0];
  // Chercher dans la pioche sans la vider
  const temp = [];
  let found = null;
  while (room.deck.length > 0) {
    const c = room.deck.pop();
    if (c.numVal === val && !found) {
      found = c; // garder la première trouvée
    } else {
      temp.push(c);
    }
  }
  // Remettre les cartes non utilisées dans la pioche (dans le bon ordre)
  temp.reverse().forEach(x => room.deck.push(x));
  return found; // null si pas trouvé — ne jamais retourner une carte random
}
...
function checkEliminated(room, player) {
  // Ne pas éliminer le Barde s'il attend encore de choisir sa carte PV
...
io.on('connection', socket => {
  ...
  socket.on('action', ({ code, type, targetId, extra }) => {
    ...
    // Sécurité : s'assurer que turnIdx pointe sur un joueur vivant
    let safetyCount = 0;
    ...
    if (!actor || actor.id!==socket.id) {
      // Renvoyer le bon état au joueur pour débloquer
    ...
    // Nécromancien : proposer le choix défausse UNIQUEMENT pour attaquer ou charger
    ...
    // Mage intercept — As ou Roi sur attaque ou changement bouclier (pas les charges)
  ...
  socket.on('barde_pick', ({ code, cardIndex }) => {
    ...
    // Mage intercept — As ou Roi sur TOUTE action (barde inclus)
    const mage = room.players.find(p => p.heroId==='mage' && !p.eliminated);
  ...
  socket.on('barde_pv_pick', ({ code, cardIndex }) => {
    ...
    broadcastState(room);
    // Si Paladin en attente, lancer sa riposte maintenant
  ...
  socket.on('voleuse_exchange', ({ code, myType, myIdx, theirType, theirIdx, targetId }) => {
    ...
    // Barde d'abord si en attente — la logique PV est gérée dans handleAction via bardePendingPvValue
    // On ne fait rien ici, le prompt barde sera lancé automatiquement après le calcul des dégâts

    // Paladin après voleuse
    if (paladinTargetId) {
  ...
  socket.on('request_state', ({ code }) => {
    ...
    const player = room.players.find(p=>p.id===socket.id);
    if (!player) return;
    // Renvoyer l'état privé au joueur
    socket.emit('state_update', { state: privateState(room, socket.id) });
    // Si c'est son tour, lui redonner la main
    const actor = room.players[room.turnIdx];
...
function handleAction(room, actor, type, targetId, drawn, extra) {
  ...
  if (aType==='attack') {
    ...
    if (!isOgreKill && atkVal<=shieldVal) {
      ...
      // Pour le Barde : totalPV() est 0 tant qu'il n'a pas choisi sa carte,
      // mais il lui reste bardePendingPvValue PV en attente
      ...
      // Révéler bouclier si perd des PV (pas pour l'espionne elle-même)
      ...
      // Bouclier révélé seulement si : pas espionne ET shieldRevealed
      ...
      if (target.heroId==='barde' && didDamage && !target.eliminated && target.bardePendingPvValue !== undefined) {
        ...
        // Snapshot de la défausse AVANT d'y ajouter drawn (pour éviter que drawn apparaisse dans les choix)
        ...
        if (discardMatches.length > 0) {
          ...
          target.bardePvPaladinPending = paladinPending; // stocker pour après
          ...
          if (fromPioche) {
            ...
            broadcastPopup(room,'🎵',`${target.name} (Barde)`,`Aucun ${nv} en défausse — carte aléatoire`,null,'neutral');
          }
        }
      }
      ...
      if (actor.heroId==='voleuse' && !target.eliminated) {
        ...
        const ps=io.sockets.sockets.get(actor.id);
        const isEsp = actor.heroId==='espionne';
        const tShield = (espionneInGame && !isEsp && !target.shieldRevealed && target.heroId!=='espionne') ? null : target.shield;
        const mShield = (espionneInGame && !isEsp && !actor.shieldRevealed) ? null : actor.shield;
        if (ps) ps.emit('voleuse_prompt',{targetId:target.id,targetName:target.name,targetPV:target.pv,targetShield:tShield,myPV:actor.pv,myShield:mShield});
      ...
      // Barde défenseur : le prompt PV est géré via bardePendingPvValue dans le bloc ci-dessous
...
function launchPaladinRiposte(room, paladin, victim) {
  ...
  setTimeout(()=>{
    ...
    setTimeout(()=>{
      ...
      if(checkGameOver(room)){ room.discard.push(rc2); return; } // victim mort → fin de partie
      setTimeout(()=>{
        if(victim.eliminated){ room.discard.push(rc2); nextAliveTurn(room); broadcastState(room); setTimeout(()=>startTurn(room),5500); return; }
        ...
        setTimeout(()=>{
          ...
          checkEliminated(room,victim);
          if(!checkGameOver(room)){ broadcastState(room); nextAliveTurn(room); setTimeout(()=>startTurn(room),5500); }
        },2000);
      },5500);
    },2000);
  },5500);
...
function makePlayer(id, name, avatar=null) {
  return { id, name, avatar, wins:0, pv:[], shield:null, charges:[], drawnCard:null, eliminated:false,
    ...
    potions:[], woundMarkers:{}, bardePvChoices:null, bardePendingPvValue:undefined, bardePvSuspendedActor:null, bardePvPaladinPending:null, shieldRevealed:false,
...
function leaveRoom(socket, code) {
  ...
  if (room.status==='playing') {
    // En jeu : éliminer le joueur plutôt que de le retirer
    leavingPlayer.eliminated=true;
    leavingPlayer.pv.forEach(c=>room.discard.push(c));
    leavingPlayer.charges.forEach(c=>room.discard.push(c));
    if (leavingPlayer.shield) room.discard.push(leavingPlayer.shield);
    leavingPlayer.pv=[]; leavingPlayer.charges=[]; leavingPlayer.shield=null;
    io.to(code).emit('player_eliminated',{playerId:leavingPlayer.id, playerName:leavingPlayer.name});
    ...
    if (room.players.length===0) { delete rooms[code]; socket.leave(code); return; }
