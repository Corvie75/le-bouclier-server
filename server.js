...
function drawCard(room) {
  const wasEmpty = room.deck.length === 0;
  if (wasEmpty) {
    ...
    room.discard = [];
    // Option Dragon : chaque joueur subit une attaque
    if (room.optDragon && room.deck.length > 0) {
      ...
      // Déclencher les attaques Dragon après le popup (3.5s)
      setTimeout(()=>dragonAttack(room), 3500);
    }
  }
...
function dragonAttack(room) {
  ...
  // Commencer par le joueur suivant dans l'ordre du tour
  const startIdx = (room.turnIdx + 1) % room.players.length;
  let idx = startIdx;
  let count = 0;
  const attacked = [];
  while (count < room.players.length) {
    const p = room.players[idx];
    if (!p.eliminated && room.deck.length > 0) {
      const card = room.deck.pop();
      const sv = getShieldVal(p);
      const dmg = card.numVal > sv ? card.numVal - sv : 0;
      if (dmg > 0) applyDamage(room, p, dmg);
      attacked.push({ name:p.name, card:cStr(card), dmg, sv });
  ...
  const detail = attacked.map(a => `${a.name}: ${a.card}${a.dmg>0?` −${a.dmg}PV`:' bloqué'}`).join(' · ');
  io.to(room.code).emit('action_popup',{emoji:'🐉',main:'Attaque du Dragon !',detail,result:null,resultType:'dmg'});
  if (!checkGameOver(room)) { broadcastState(room); }
}
  ...
  while (rem > 0 && target.pv.length > 0) {
    ...
    if (rem >= w.numVal) {
      ...
      if (target.heroId === 'barde') {
        // Barde choisit sa carte de remplacement — géré séparément via barde_pv_prompt
        target.bardePendingPvValue = nv;
      } else {
        // Tout le monde : trouver la carte de la bonne valeur dans défausse/pioche
...
io.on('connection', socket => {
  ...
  socket.on('barde_pv_pick', ({ code, cardIndex }) => {
    ...
    // Retirer la carte choisie de la défausse (la bonne occurrence)
    ...
    room.waitingFor = null;
    io.to(room.code).emit('waiting_for', null);
    broadcastPopup(room,'🎵',`${actor.name} choisit sa carte PV`,`${cStr(chosen)} depuis la défausse`,null,'neutral');
    broadcastState(room);
  });
  ...
  if (aType==='attack') {
    ...
    if (!isOgreKill && atkVal<=shieldVal) {
      ...
      if (target.heroId==='barde' && didDamage && !target.eliminated && target.bardePendingPvValue !== undefined) {
        ...
        // Chercher valeur exacte dans la défausse
        const discardMatches = room.discard.filter(c => c.numVal === nv);
        if (discardMatches.length > 0) {
          target.bardePvChoices = discardMatches;
          room.waitingFor = { playerId:target.id, playerName:target.name, heroEmoji:'🎵', reason:`choisit une carte de ${nv} PV dans la défausse…` };
          io.to(room.code).emit('waiting_for', room.waitingFor);
          ...
          if (bs) bs.emit('barde_pv_prompt', { choices:discardMatches, pvValue:nv, fromDiscard:true });
        } else {
          // Pas dans la défausse → pioche aléatoire de la valeur exacte
          const fromPioche = findCardOfValue(room, nv);
          if (fromPioche) {
            target.pv.push(fromPioche);
            broadcastPopup(room,'🎵',`${target.name} (Barde)`,`Aucun ${nv} en défausse — carte prise dans la pioche`,null,'neutral');
          }
        }
      }
...
function makePlayer(id, name, avatar=null) {
  return { id, name, avatar, wins:0, pv:[], shield:null, charges:[], drawnCard:null, eliminated:false,
    ...
    potions:[], woundMarkers:{}, bardePvChoices:null, bardePendingPvValue:undefined, shieldRevealed:false,
