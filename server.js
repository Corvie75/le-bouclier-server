...
function handleAction(room, actor, type, targetId, drawn, extra) {
  ...
  if (aType==='attack') {
    ...
    if (!isOgreKill && atkVal<=shieldVal) {
      ...
      // Révéler bouclier si perd des PV et espionne en jeu
    ...
    const shieldDetail = actor.heroId==='espionne'
      ? `${actor.name} cache le bouclier de ${target.name}`
      : wasHidden
...
function makePlayer(id, name, avatar=null) {
  return { id, name, avatar, wins:0, pv:[], shield:null, charges:[], drawnCard:null, eliminated:false,
    ...
    potions:[], woundMarkers:{}, bardePvChoices:null, shieldRevealed:false,
